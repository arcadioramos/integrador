...
public class IniciarSesionImpl {
	...
	public Object LogIn(Object u) throws FileNotFoundException {
		...
		try {
			...
			String query = "SELECT id, nombre, apellidos, correo, password, tipoUsuario, imagen FROM usuario WHERE correo = '" + usr.getCorreo() + "' and password = '" + usr.getPassword() + "' LIMIT 1;";
	...
	public Object Session() {
		...
		try {
			Connection conx = Conexion.getConnection();
			String query = "SELECT idUsuario, nombre, apellidos, correo, password, tipoUsuario, imagen FROM session WHERE id=(SELECT MAX(id) FROM session) LIMIT 1;";
			st = conx.prepareStatement(query);
			ResultSet rs = st.executeQuery();
			if (rs.next()) {
